...
# 对以下代码修改配置，将usrp_source改为两个端口接收，并且保存两个端口的数据。
# 但是为了简便，后面只改为只让第一个天线收到的信号经过后续block，所以运行的时候只可在终端的输出看到第一个天线的接收情况，
# 所以得到的output.bits也是第一个天线的解调结果。
# 第二个天线的用保存下的raw data文件再经过offline模式去过单个接收信号的代码以此来解调
...
#--------------------------------------------------------------------
            #双端口接收信号的配置
            # Use two RX channels
            stream_args = uhd.stream_args(cpu_format, wire_format, args=stream_args, channels=list(range(0, 2)))
            source = uhd.usrp_source(dev_addr + "," + dev_args, stream_args)

            # Configure the subdevice spec to use A:0 and B:0 for daughterboards
            source.set_subdev_spec('A:0 B:0', 0)

            # Set sample rate
            source.set_samp_rate(self._input_sample_rate)

            # Configure both RX channels (0 and 1)
            for ch in range(2):
                source.set_center_freq(self._center_frequency, ch)

                if 'gain' in d:
                    gain = int(d['gain'])
                    source.set_gain(gain, ch)
                    print(f"Gain (channel {ch}):", source.get_gain(ch), '(Requested %d)' % gain, file=sys.stderr)

                if 'bandwidth' in d:
                    bandwidth = int(d['bandwidth'])
                    source.set_bandwidth(bandwidth, ch)
                    print(f"Bandwidth (channel {ch}):", source.get_bandwidth(ch), '(Requested %d)' % bandwidth, file=sys.stderr)
                else:
                    source.set_bandwidth(0, ch)
                    print(f"Warning: Setting bandwidth (channel {ch}) to", source.get_bandwidth(ch), file=sys.stderr)

                # Use RX2 antenna for both channels
                source.set_antenna("RX2", ch)
                print(f"Antenna (channel {ch}):", source.get_antenna(ch), '(Requested RX2)', file=sys.stderr)           
#--------------------------------------------------------------------------
...
#------------如果配置文件中没有source block，那就不需要，是offline，直接读取信号的文件，然后进行后续模块-------------------
        ...
        # 初始化 file_sink
        self._file_sink_0 = blocks.file_sink(gr.sizeof_gr_complex, "/home/ddh/ssd2/iridium_data/antenna_0_after_source.dat")
        self._file_sink_1 = blocks.file_sink(gr.sizeof_gr_complex, "/home/ddh/ssd2/iridium_data/antenna_1_after_source.dat")

        # 连接 USRP 源头的两个通道到各自的 file_sink
        tb.connect((self.source, 0), self._file_sink_0)  # 连接通道0到file_sink_0
        tb.connect((self.source, 1), self._file_sink_1)  # 连接通道1到file_sink_1
...
#-%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        tb.connect((self.source, 0), self._fft_burst_tagger) #这里改为只让第一个天线收到的信号经过后续block
        
        
        #2.后接一个file sink模块把这个fft_burst_tagger的输出保存下来
        self._file_sink_3 = blocks.file_sink(gr.sizeof_gr_complex, "/home/ddh/ssd2/iridium_data/antenna_0_after_fft_burst_tagger_file.dat")
        tb.connect(self._fft_burst_tagger, self._file_sink_3)
